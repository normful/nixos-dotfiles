...
| Provider pricing (for `openrouter/*` and `vercel-ai-gateway/*` enabledModels) | `curl -s https://models.dev/api.json -o /tmp/models_dev.json` | Parse `openrouter.models` / `vercel.models` `["<model-id>"].cost`. |
| Enabled models list | `chezmoi/dot_pi/agent/settings_mac.json` (via `settings.json.tmpl`) key `enabledModels` | order in file **kept as-is** — only `modelOverrides` objects are sorted descending by `⟐`. |
...
| `hy3` | `hy3` | — |
| *(no AA slug)* | `deepseek-flash` (V4.1 Flash), `hy4-preview`, `omen-alpha` | — (AA null/missing → rate-limit-only name) |
...
Extract from go.mdx section "Usage limits". Use these values for calculations (27 models as of 2026-09-10):

| Model | Requests per 5hr | Requests per month | Per Hour (÷5) | Per Min (÷5÷60) |
| Grok 4.6 | 169 | 845 | 34 | 0.6 |
| GPT 5.6 Luna | 2050 | 10250 | 410 | 6.8 |
| GLM-5.3-Flash | 6320 | 31580 | 1264 | 21.1 |
...
| DeepSeek V4.1 Flash | 6500 | 32500 | 1300 | 21.7 |
| DeepSeek V4 Pro | 1050 | 5200 | 210 | 3.5 |
| DeepSeek V4 Flash | 13000 | 65000 | 2600 | 43.3 |
| DeepSeek V4 Flash Vision Exp | 6500 | 32500 | 1300 | 21.7 |
...
MODELS = {
    ...
    "GLM-5.3-Flash":     {"reqs_5hr": 6320,  "reqs_mo": 31580},
    ...
    "DeepSeek V4.1 Flash": {"reqs_5hr": 6500, "reqs_mo": 32500},
    "DeepSeek V4 Pro":   {"reqs_5hr": 1050,  "reqs_mo": 5200},
    "DeepSeek V4 Flash": {"reqs_5hr": 13000, "reqs_mo": 65000},
    "DeepSeek V4 Flash Vision Exp": {"reqs_5hr": 6500, "reqs_mo": 32500},
...
For the 20 models listed in `chezmoi/dot_pi/agent/settings_mac.json` `enabledModels` (`settings.json.tmpl` includes either `settings_mac.json` or `settings_exeDevVm.json`; the mac list is the superset), create **per-provider** `modelOverrides` in `chezmoi/dot_pi/agent/models.json` (`providers.<provider>.modelOverrides` keyed by bare model ID, e.g. `providers["aihubmix-oc"].modelOverrides["coding-glm-5.3-flash"]`, `providers["openrouter"].modelOverrides["inception/mercury-2.5"]`). Same `⟐`/`●` suffix format as opencode-go, but **pricing source differs** (not `go.mdx`):
...
- `vercel-ai-gateway/*` → `https://models.dev/api.json` `vercel.models` cost (fall back to `openrouter.models` when the model is absent from the `vercel` section)
- `opencode-go/*` already covered by go.mdx table — reuse same `reqs_mo` for those entries; do **not** recompute with uniform mix

Existing `modelOverrides` entries for models that are no longer enabled are kept as-is (do not prune); add/refresh only the entries for the current `enabledModels`.

For non-opencode (aihubmix/openrouter/vercel), compute synthetic monthly capacity with a **uniform token mix `500 input / 50,000 cached / 200 output`** for *all* models and a **$60 monthly budget** (same budget as Go):
...
Free models (`coding-glm-5.2-free` $0/$0) have `cost_per_req = 0` → no `●` product, only `⟐ coding` if available. Models with **no matching AA slug at all** (`mercury-2.5`, `deepseek-v4.1-flash`) get no `⟐`/`●` suffix — the name stays `Display (reqs: …)`. Documented fallbacks (`agnes-2-5-pro-alpha` for `agnes-2.5-flash`, `qwen3-8-flash-next` for `qwen3.8-flash`, `deepseek-v4-flash-vision` for `deep-deepseek-v4-flash-vision-exp`) are still applied.
...
| `enabledModels` entry | AA Slug (coding) | Pricing source key |
| `opencode-go/muse-spark-1.3-contributor` | `muse-spark-1-3` 75.8 | go.mdx `Muse Spark 1.3 Contributor` 45300/5hr |
| `opencode-go/deepseek-v4-flash` | `deepseek-v4-flash` 69.1 (0731 Max) | go.mdx 13000/5hr |
| `aihubmix-oc/deepseek-v4-flash-0731-fast` | `deepseek-v4-flash` 69.1 | aihubmix `deepseek-v4-flash-0731-fast` $0.28/$1.4 cache 0.07 |
| `aihubmix-oc/deep-deepseek-v4-flash-vision-exp` | `deepseek-v4-flash-vision` 65 *fallback* (no `-exp` slug in AA) | aihubmix `deepseek-v4-flash-vision-exp` $0.155/$0.62 cache 0.0031 |
| `aihubmix-oc/coding-glm-5.3-flash` | `glm-5-3-flash` 71.5 | aihubmix `glm-5.3-flash` $0.11268/$0.39438 cache 0.02817 (no exact `coding-glm-5.3-flash` key in catalog) |
...
| `aihubmix-oc/agnes-2.5-flash` | `agnes-2-5-pro-alpha` 58.8 *fallback* (no `agnes-2-5-flash` in AA) | aihubmix `agnes-2.5-flash` $0.03/$0.15 |
...
| `vercel-ai-gateway/inclusionai/ling-3.0-flash` | `ling-3-0-flash` 50.6 | models.dev `vercel` `inclusionai/ling-3.0-flash` $0.06/$0.18 cache 0.012 |
| `vercel-ai-gateway/deepseek/deepseek-v4-flash-0731` | `deepseek-v4-flash` 69.1 | models.dev `vercel` `deepseek/deepseek-v4-flash-0731` $0.076/$0.153 cache 0.014 |
| `vercel-ai-gateway/zai/glm-5.3-flash` | `glm-5-3-flash` 71.5 | models.dev `vercel` `zai/glm-5.3-flash` $0.15/$0.5 cache 0.03 |
| `opencode-go/omen-alpha` | *(none)* | stale (dropped from live go.mdx) — kept at 11600/5hr |
| `opencode-go/mimo-v2.5` | `mimo-v2-5-0424` 56.8 | go.mdx 30100/5hr |
| `opencode-go/hy3` | `hy3` 58.8 | go.mdx 4300/5hr |
| `opencode-go/qwen3.8-flash` | `qwen3-8-flash-next` 73.1 *fallback* | go.mdx 5400/5hr |
| `openrouter/inception/mercury-2.5` | *(none — no `mercury-2-5` in AA)* | models.dev `openrouter` `inception/mercury-2.5` $0.04/$0.15 cache 0.004 |
| `aihubmix-oc/mercury-2.5` | *(none — AA only has `mercury-2`)* | aihubmix `mercury-2.5-preview` $0.2/$0.75 cache 0.02 |
| `aihubmix-oc/deepseek-v4.1-flash` | *(none — no `deepseek-v4-1-flash` in AA)* | aihubmix `deepseek-v4.1-flash` $0.155/$0.62 cache 0.0031 |
...
ENABLED_MODELS = [
  ...
  "aihubmix-oc/mercury-2.5",
  "aihubmix-oc/deepseek-v4.1-flash",
]
...
ENABLED_AA = {
  ...
  "vercel-ai-gateway/deepseek/deepseek-v4-flash-0731": "deepseek-v4-flash",
  "vercel-ai-gateway/zai/glm-5.3-flash": "glm-5-3-flash",
  "opencode-go/mimo-v2.5": "mimo-v2-5-0424",
  "opencode-go/hy3": "hy3",
  "opencode-go/qwen3.8-flash": "qwen3-8-flash-next",
  # omen-alpha, mercury-2.5, deepseek-v4.1-flash: no AA slug -> no ⟐ suffix
}
# pricing helpers
# aihubmix pricing -> cost_per_req_aih
# models.dev `vercel` (fallback `openrouter`) pricing -> cost_per_req_or
...
3. **Swapped rate limits**: Verify cheaper models have MORE requests/hr than expensive ones. For example Qwen3.7 Plus should be ~860/hr while Qwen3.7 Max is ~34/hr, and Qwen3.6 Plus ~660/hr vs Qwen3.8 Max ~32/hr. If you see the expensive Max with higher limits, you swapped rows.
...
8. **Missing new models**: The catalog has 27 rows in the go.mdx "Estimated requests" table. Row *count* matching is not enough — check membership too. As of 2026-09-10 that means `deepseek-flash` (DeepSeek V4.1 Flash, 6500/5hr) is in and `omen-alpha` is out (kept as a stale override). If `go.mdx` row count ≠ `len(MODELS)`, update the skill — do not silently drop new models like `grok-4.6`, `glm-5.3-flash`, `longcat-2.0`, `qwen3.8-flash`, `deepseek-v4-flash-vision-exp`, `hy4-preview`, `muse-spark-1.3-contributor`. Watch for rate changes on existing rows too (`glm-5.3-flash` 1580→6320, `deepseek-v4-flash` 7600→13000, `deepseek-v4-flash-vision-exp` 3800→6500 per 5hr; `grok-4.5` dropped from live docs 2026-09-05 but kept as stale override; `qwen3.7-max` rates changed 340→170/5hr).
