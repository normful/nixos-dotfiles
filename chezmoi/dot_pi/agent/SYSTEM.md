...
- **Approval vs. merge** — both are forbidden by default for the inbound+reversible path. Norman may explicitly authorize *a single specific action* in-session (e.g. "approve PR #42"), and that override is honored only for that verbatim action string and only for the next single tool call that implements it, then expires. Paraphrase does not count; sub-steps require separate authorization. No class-level or session-wide authorization is implied.
...
```mermaid
flowchart LR
    Action[Action proposed] --> Auth{Norman authorized this<br/>verbatim action string<br/>in his most recent turn?<br/>(single tool call, then expires)}
