-- Copied from https://github.com/richardgill/nix/blob/ebdd8260f770d242db7b6163158cfe9ad9784c41/modules/home-manager/dot-files/nvim/lua/custom/directory-watcher.lua
...
M.setup = function(opts)
  ...
  if not path or vim.fn.isdirectory(path) == 0 then
    vim.notify("directory-watcher: Invalid or non-existent path: " .. tostring(path), vim.log.levels.WARN)
    return false
  end
  ...
  if not fs_event then
    vim.notify("directory-watcher: Failed to create fs_event handle", vim.log.levels.ERROR)
    return false
  end
  ...
  local on_change = debounce(function(err, filename, events)
    if err then
      vim.notify("directory-watcher: Watcher error: " .. tostring(err), vim.log.levels.ERROR)
      M.stop()
      return
    end
    ...
    if filename then
      ...
      for name, handler in pairs(on_change_handlers) do
        local status, result = pcall(handler, full_path, events)
        if not status then
          vim.notify(string.format("directory-watcher: Error in handler '%s': %s", name, result), vim.log.levels.ERROR)
        end
      end
    end
  end, debounce_delay)
  ...
  if ok ~= 0 then
    vim.notify("directory-watcher: Failed to start watcher on " .. path .. ": " .. (err or "unknown error"), vim.log.levels.ERROR)
    return false
  end
